 ...
 * responses alone.
 *
 * Freshness and validation rest on what a response states, that is `Cache-Control` `no-store`, `no-cache` and
 ...
 *
 * The generated middleware replays what it holds for a `GET` or `HEAD` exchange while the freshness the origin server
 * stated lasts, revalidates it with the stored validators once that time is up, replaying the held content again when
 ...
 *
 * Every numeric option states an upper bound and is removed by a value less than or equal to `0`, as the defaults do:
 * entries are then retained for the life of the middleware and freshness stays in the sole control of the origin
 * server.
 *
 ...
 * @param options.ttl The time a stored response is reused for, in milliseconds: caps the freshness lifetime computed
 *     from the response header fields, never extending it
